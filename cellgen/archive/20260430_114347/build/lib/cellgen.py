...
def find_best_config(needed_dp: int, needed_wd: int,
    ...
    # 找最优DP选项
    # 规则: 优先选择CELL值最接近needed_dp的，其次才是最少拼接
    best_dp = None
    best_dp_diff = float('inf')  # CELL值与needed_dp的差值

    for dp, num_slices, total in dp_options:
        diff = abs(dp - needed_dp)  # CELL值与需求的差值
        if diff < best_dp_diff:
            best_dp_diff = diff
            best_dp = dp
            best_dp_slices = num_slices
        elif diff == best_dp_diff and num_slices < best_dp_slices:
            best_dp = dp
            best_dp_slices = num_slices
