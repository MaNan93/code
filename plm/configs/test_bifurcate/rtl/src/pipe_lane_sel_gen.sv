...
// Group pipe_lane_sel_sync mode: sync (SYNC=1, safe across clock domains)
...
module pipe_lane_sel_gen #(
    ...
    output lane_sel_t sel_tgt,   // final effective selection after pipe_lane_sel_sync synchronization; used by clk_mux/data mux
    ...
    pipe_lane_sel_sync #(.N(2), .SYNC(1'b1)) u_sel_sync_g1 (
        .branch_clk   ({ctrl_pclk[3], ctrl_pclk[0]}),
        .branch_rst_n ({ctrl_rst_n[3], ctrl_rst_n[0]}),
        .tgt          (dec_tgt.g1),
        .en           (sel_tgt.g1)
    );
    ...
    pipe_lane_sel_sync #(.N(3), .SYNC(1'b1)) u_sel_sync_g3 (
        .branch_clk   ({ctrl_pclk[2], ctrl_pclk[1], ctrl_pclk[0]}),
        .branch_rst_n ({ctrl_rst_n[2], ctrl_rst_n[1], ctrl_rst_n[0]}),
        .tgt          (dec_tgt.g3),
        .en           (sel_tgt.g3)
    );
