...
module pipe_lane_sel_gen #(
    ...
    pipe_lane_sel_sync #(.N(2), .SYNC(1'b1)) u_sel_sync_g2 (
        .branch_clk   ({ctrl_pclk[4], ctrl_pclk[0]}),
        .branch_rst_n ({ctrl_rst_n[4], ctrl_rst_n[0]}),
        .tgt          (dec_tgt.g2),
        .en           (sel_tgt.g2)
    );

    // G4 lane4~4: CtrlBeta, CtrlZeta
    pipe_lane_sel_sync #(.N(2), .SYNC(1'b1)) u_sel_sync_g4 (
        .branch_clk   ({ctrl_pclk[5], ctrl_pclk[1]}),
        .branch_rst_n ({ctrl_rst_n[5], ctrl_rst_n[1]}),
        .tgt          (dec_tgt.g4),
        .en           (sel_tgt.g4)
    );
