...
# Parser function
#----------------------------------------------------------
parser_dir = G["InstallPath"] + '/Parser'
parser_c_file = parser_dir + '/Parser.c'

def _parser_bin_candidates():
    return [parser_dir + '/parser', parser_dir + '/parser.exe']

def _pick_parser_bin():
    for p in _parser_bin_candidates():
        if os.path.isfile(p):
            return p
    # fallback to historical default name
    return parser_dir + '/parser'

parser_bin = _pick_parser_bin()
...
def ensure_c_parser_up_to_date(force_rebuild=False):
    global parser_bin
    if force_rebuild:
        compile_c_parser()
        return
    ...
    if not os.path.isfile(parser_bin):
        ...
        return
    try:
        src_mtime = os.path.getmtime(parser_c_file)
        bin_mtime = os.path.getmtime(parser_bin)
    except OSError:
        compile_c_parser()
        return
...
def parser_from_file_list(compile_define_list, incdir_list, design_list, log_path, ftype = "tmp", force_rebuild = False):
    parser_file_list = []
    ...
    ensure_c_parser_up_to_date(force_rebuild = force_rebuild)
    # do c parser
